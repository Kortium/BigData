...
def convert_data_to_table_format():
    ...
    parser = Parser([])
    parser.parse_process(SCRAPPED_FILE, TABLE_FORMAT_FILE)
    # Your code here
...
def stats_of_data():
    logger.info("stats")
    player_DF = pd.read_csv(TABLE_FORMAT_FILE)
    print("Info:")
    print(player_DF.info())
    print("Description:")
    print(player_DF.describe())
    print("\nPrevious achievments")
    print(player_DF["achievment"].value_counts())
    print("\nMost popular items from all slots")
    print(player_DF['item0'].append(player_DF['item1']).append(player_DF['item2']).append(player_DF['item3']).append(player_DF['item4']).append(player_DF['item5']).value_counts().head())
    print("\nMost popular items from all slots wins")
    winners = player_DF[player_DF['win']]
    not_winners = player_DF[~player_DF['win']]
    print(winners['item0'].append(winners['item1']).append(winners['item2']).append(winners['item3']).append(winners['item4']).append(winners['item5']).value_counts().head())
    print("\nMost popular items from all slots loose")
    print(not_winners['item0'].append(not_winners['item1']).append(not_winners['item2']).append(not_winners['item3']).append(not_winners['item4']).append(not_winners['item5']).value_counts().head())
    print("\nFirst blood win")
    print(winners['firstBlood'].value_counts())
    print("\nFirst blood loose")
    print(not_winners['firstBlood'].value_counts())
